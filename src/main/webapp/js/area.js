...
/**
 * 存入数据库只需要存入 Area 就行了 ，使用 getCarStoreArea()将代码转换为三级区域名称
 * @param strP 省份
 * @param strC 城市
 * @param strA 区域
 * @constructor
...
function AreaCtrlNormal(strP, strC, strA){
	...
	if (this.level>1) {
		this.html += '<span>'+strC+'<select class="form-control" name="City" id="city'+this.randomNum+'" '+onchgCityStr+' ></select></span>';
	}
	if (this.level>2) {
		this.html += '<span>'+strA+'<select class="form-control" name="Area" id="area'+this.randomNum+'"></select></span>';
	}
 ...
 * @param key 6位区域代码
 */
function getCarStoreArea(key){
	let provName, cityName, areaName;
	let province, city, area;
	if (key!==''){
		province = key.substring(0, 2) + '0000';
		city = key.substring(0, 4) + '00';
		area = key;
	} else {
		provName = '店铺还未更新地区';
		cityName = '店铺还未更新地区';
	...
	provName = __alldiv[province][0];
	cityName = __alldiv[city][0];
	areaName = __alldiv[area][0];
	document.write('<p class="d-inline">'+provName+' | '+cityName+' | '+areaName+'</p>')
}