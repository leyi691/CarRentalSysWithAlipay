...
<%@ page import="cn.edu.nbut.jerry.service.UserService" %>
<%@ page import="cn.edu.nbut.jerry.service.impl.UserServiceImpl" %>
    ...
    <div class="row">
        <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
            <div class="position-sticky pt-3">
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="<%=basePath%>/doSysAdmin?op=shopManage&do=list">
                            <span data-feather="order"></span>
                            门店管理
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link active" href="<%=basePath%>/doSysAdmin?op=userManage&do=list">
                            <span data-feather="order"></span>
                            用户管理
                        </a>
                    </li>
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                ...
                <form class="row" id="thisForm1" name="thisForm" method="post">
                    <div class="input-group" role="group">
                        <label>
                            <input name="searchTxt" type="text" class="form-control" placeholder="按用户昵称搜索">
                        </label>
            ...
            <%
                ...
                int pageSize = NamesConfig.ONE_PAGE_SIZE_LONG;
                // 显示所有的门店信息
                ...
                int pageNow = 0;
                if (searchText != null) {
                    users = (List<User>) CommonUtils.getSpecificPage(pageNow, userService.searchUserLikeNicknameForSysAdmin(searchText), pageSize);
                } else if (getPageNow != null) {
                    pageNow = Integer.parseInt(getPageNow);
                    users = (List<User>) CommonUtils.getSpecificPage(pageNow, userService.getAllUser(), pageSize);
                } else if (request.getAttribute("users") != null){
                    users = (List<User>) request.getAttribute("users");
                } else {
                    users = (List<User>) CommonUtils.getSpecificPage(0, userService.getAllUser(), pageSize);
                }
            %>
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tr>
                        ...
                        <td class=""><%=user.getUsername()%></td>
<%--                        TODO 这里的密码要不要明文显示呢？             --%>
                        <td class=""><%=user.getPassword()%></td>
                        <td class=""><%=user.getNickname()%></td>
                        <td class=""><%=simpleDateFormat.format(user.getRegisterDate())%></td>
                        <td class=""><%=user.getPhoneNumber()%></td>
                        <td class=""><%=user.getIdType()%></td>
                        <td class=""><%=user.getIdNumber()%></td>
                        <td class="">
                            <script type="text/javascript">
                                getCarStoreArea('<%=user.getArea()%>')
                            </script>
                            | <%=user.getAddress()%>
                        </td>
                        <td class=""><%=user.getSex()%></td>

                        <td class="">
                            <%--                            使用<a> 标签的id 来暂时保存carId 我真是个天才！！！！！！！！                       --%>
                            <form id="thisForm<%=user.getUserId()%>" name="thisForm" method="post">
                                <input type="hidden" name="toEditCarStoreId" value="<%=user.getUserId()%>">
                            </form>
                            <a href="<%=basePath%>/doSysAdmin?op=userManage&do=delete&userId=<%=user.getUserId()%>" class="btn btn-outline-danger">删除</a>
                        </td>
                    </tr>
                    <%
                        }
            ...
            <div class="btn-group float-end mb-3">
                <%
                    ...
                    // 下一页
                    int next = pageNow >= pageSize ? pageSize : pageNow + 1;
                    if (pageNow != 0) {
                %>
                <a class="btn btn-primary" href="<%=basePath%>/jsps/sysAdmin/sysAdminManageUser.jsp?&pageNow=<%=front%>">上一页</a>
                <%
                    }
                    if (pageNow < userService.getAllUser().size() / pageSize) {
                %>
                <a class="btn btn-primary" href="<%=basePath%>/jsps/sysAdmin/sysAdminManageUser.jsp?pageNow=<%=next%>">下一页</a>
                <%
                    }
                %>
            </div>
                    ...
                    <div class="container">
                        <div class="row">
                            ...
                            <div class="col-6 mt-2">
                                ...
                                <input type="radio" name="inputStatus" id="inputStatusOpenInsert" class="form-check-input" value="开业中" required>
                                <label for="inputStatusOpenInsert" class="form-label my-0 m-1 text-success">开业中</label>

                                <input type="radio" name="inputStatus" id="inputStatusSuspendedInsert" class="form-check-input" value="暂停营业" required>
                                <label for="inputStatusSuspendedInsert" class="form-label my-0 m-1 text-warning">暂停营业</label>

                                <input type="radio" name="inputStatus" id="inputStatusTerminatedInsert" class="form-check-input" value="永久停业" required>
...
<%--                            &lt;%&ndash;                            <div class="col-6 mb-5">&ndash;%&gt;--%>
