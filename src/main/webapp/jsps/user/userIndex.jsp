    ...
    <div class="row">
        <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
            <div class="position-sticky pt-3">
                <ul class="nav flex-column">
                    <li class="nav-item">
                        <a class="nav-link active" aria-current="page" href="<%=basePath%>/doUser?op=user">
                            <span data-feather="home"></span>
                            个人主页
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="<%=basePath%>/doUser?op=orderList">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                <h1 class="h2">个人主页</h1>
                <button type="button" class="btn btn-primary" onclick="doEdit()">点我开始修改你的个人信息</button>
            </div>
<%
    // 获取servlet传来的个人信息
...
<%--            <%=basePath%>/doUser?op=userIndexEdit--%>
            <form action="<%=basePath%>/doUser?op=userIndexEdit" method="POST" enctype="multipart/form-data">
                <div class="container">
                    <div class="row">
                <!--  隐式传送userId  -->
                        <input type="hidden" name="inputUserId" value="<%=user.getUsername()%>">
                        ...
                        <div class="col-6">
                            ...
                            <div class="mb-3">
                                性别：
                                <input type="radio" name="inputSex" id="inputSexNone"
                                       class="form-check-input" value="无" disabled>
                                <label for="inputSexNone" class="form-label my-0 m-1">不愿透露</label>


                                <input type="radio" name="inputSex" id="inputSexMan"
                                       class="form-check-input" value="男" disabled>
                                <label for="inputSexMan" class="form-label my-0 m-1">男</label>


                                <input type="radio" name="inputSex" id="inputSexWoman"
                                       class="form-check-input" value="女" disabled>
                                <label for="inputSexWoman" class="form-label my-0 m-1">女</label>

                                <!--  先等待input标签加载完成，再将其选中  -->
                                <script type="text/javascript">sexSelectInit("<%=user.getSex()%>")</script>

                            </div>
                            <div class="mt-3">
                                <label for="inputPhoneNumber" class="form-label my-0">手机号：</label>
                            ...
                            <div class="mt-3">
                                ...
                                <select name="inputIdType" id="inputIdType" class="form-control" disabled>
                                    <option id="residentIdentityCard" value="居民身份证">居民身份证</option>
                                    <option id="certificateOfOfficers" value="军官证">军官证</option>
                                    <option id="certificateOfArmedPoliceOfficer" value="武警警官证">武警警官证</option>
                                    <option id="soldierCertificate" value="士兵证">士兵证</option>
                                    <option id="passport" value="护照">护照</option>
                                </select>
                                <script type="text/javascript">idTypeSelectInit("<%=user.getIdType()%>")</script>
                            </div>

                            <div class="mt-3">
                                <label for="inputIdNumber" class="form-label my-0">证件号码：</label>
                                <input type="text" name="inputIdNumber" id="inputIdNumber" class="form-control mb-3"
                                       value="<%=user.getIdNumber()%>" disabled>
                            </div>
                        </div>
                        ...
                        <div class="col-6">
                            <div class="row justify-content-center">
                                <%
                                    ...
                                    // 因为数据库里面 <null> 和 "" 不一样
                                    if (user.getPortrait() == null || user.getPortrait().equals("") || user.getPortrait().equals("null")) {
                                        portrait = basePath + File.separator + "resources" + File.separator
                                                + "portrait-image" + File.separator + "portrait.jpeg";
                                        %>
                                <input type="hidden" value="true" name="isFirstUploadPortrait">
                                <%
                                    } else {
                                        portrait = basePath + user.getPortrait();
                                    }
                                %>
                                <img class="rounded-circle img-fluid" src="<%=portrait%>"
                                     alt="头像" style="width: 47.25%;">
                            </div>
                            <!-- <div class="bg-img w-50 h-50" style="background-image: url(portrait.jpeg);" ></div> -->
                            <div class="mt-0">
                                <label for="formFile" class="form-label my-0">上传你的头像：</label>
                                <!-- 隐式传送头像名方便修改头像的时候删除源文件 -->
                                <input type="hidden" value="<%=user.getPortrait()%>" name="originalImageName">
                                <input class="form-control" name="uploadPortrait" type="file" id="formFile" disabled>
                            </div>
                        </div>
                        ...
                        <div class="row justify-content-md-center"> <!-- row justify-content-md-center 用于居中  -->
                            <div class="col-6">
